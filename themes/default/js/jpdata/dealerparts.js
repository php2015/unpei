//全局变量信息
var global_parts = {"hashPrefix":"#parts/","make":"","series":"","year":"","model":"","maingroup":"","subgroup":"","status":0};
var global_oe = {"hashPrefix":"#search/oe/","oeno":"","make":"","status":0};
var global_name = {"hashPrefix":"#search/name/","name":"","make":"","series":"","year":"","model":"","status":0};
...
$(document).ready(function(){
    ...
    $.ajax({
         ...
         success:function(data){
	         var html = "<option value=''>厂家</option>";
	...
	$("#vehicle-make-list").change(function(){
		...
		{   $("#vehicle-year-list").attr("disabled",false);
			$('#vehicle-series-list').html("<option value=''>车系</option>");
			$('#vehicle-model-list').html("<option value=''>车型</option>");
			$('#vehicle-year-list').html("<option value=''>年款</option>");
			$('#vehicle-maingroup').html("<option value=''>主组</option>");
			$('#vehicle-group').html("<option value=''>子组</option>");
			return false;
		}
	...
	$("#vehicle-series-list").change(function()
	{
	    ...
	    $.ajax({
	         ...
	         success:function(data){
		         ...
		         if(global_parts.year && global_parts.status++ == 3){
...
//                   $('#vehicle-group').css('color','black');
      ...
      $('#oeno').change(function(){
      	  ...
      	  $('#oeno').prev('span').find('b').css({color:""});
      });
 	  ...
 	  $('#oeno-search').click(function(){
		...
		if(oeno.indexOf('*') >= 0){
			...
			if(oeno.replace(/\*/g,'').length < 3){
				$('#oeno').prev('span').find('b').css({color:"red"});
				return false;
			}
		...
		$.ajax({
 		...
 		$("#partname").change(function(){
			...
			if(partname.length < 2){
				$('#partname').prev('span').find('b').css({color:"red"});
				return false;
			}
