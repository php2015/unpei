...
class EClientScript extends CClientScript
{
	...
	/**
	 * @var not to combined file
	 */
	public $excludeScriptFiles = array();
	...
	protected function combineCssFiles()
	{
		...
		foreach ($this->cssFiles as $url => $media) {
			...
			//$cssFileName = $match[1];
			$cssFileName = basename($url);
			$file = $this->getLocalPath($url);
			if ($file === false || in_array($cssFileName, $this->excludeCssFiles)) {
				$cssFiles[$url] = $media;
			} else {
	...
	protected function combineScriptFiles($type = self::POS_HEAD)
	{
		...
		foreach ($this->scriptFiles[$type] as $url => $value) {
			...
			$scriptFileName = basename($value);
			// 是否合并脚本
			if (is_array($value) 
				|| in_array($scriptFileName, $this->excludeScriptFiles)
				|| !($file = $this->getLocalPath($url))) {
				$scriptName[] = $url;
				$scriptValue[] = $value;
			} else {
