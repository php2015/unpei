<style>
    /*.selectDiv{background: #fff; width: 200px; display: none; position: absolute; z-index: 1000; border:2px solid #39AE39;}*/
    /*.selectDiv2{background: #fff; width: 380px; display: none; position: absolute; z-index: 1000;border:2px solid #39AE39;}*/
    ...
    /*    .selected a{ background:#39AE39; color: #fff;}
          .selected a{ background:#39AE39; color: #fff;}*/
	...
	.right_A ul li.li_all{ text-indent:1em; height:26px; line-height:26px; margin:1px 4px 0; overflow: hidden}
	.right_A ul li.li_all a{ color:#676767; font-size:12px;}
        ...
        <div class="selectpop ass-items" style="clear:both; overflow:auto;">
            ...
            <div class="car_brand" style=" clear:both; overflow:auto;">
                ...
                <div class="left_A" style="float:left;">
                    <ul style="list-style:none; margin:0;padding-left:10px; height: 24px; overflow:hidden" >
                        <?php
...
<script>
    ...
    function setSelectVechile() {
        ...
        $("#make-select-index").val('');
        var vechileName = "";
        if ($("#ul-car-makes .selected3").length == 1 && !maketarget) {
            vechileName = $("#ul-car-makes .selected3 a").text();
            $("input[name=jpmall_make]").val($("#ul-car-makes .selected3 a").attr("key"));
        }
        if (vechileName != "" && $("#ul-car-series .selected3").length == 1 && !sericesarget) {
            vechileName += " " + $("#ul-car-series .selected3 a").text();
            if ($("#ul-car-series .selected3 a").attr("key") == 'all'){
                $('#make-car-m').hide();
                isshow = false;
            }
            else
                $("input[name=jpmall_series]").val($("#ul-car-series .selected3 a").attr("key"));
        }
        if (vechileName != "" && $("#ul-car-year .selected3").length == 1 && !yeartarget) {
            vechileName += " " + $("#ul-car-year .selected3 a").text();
            if ($("#ul-car-year .selected3 a").attr("key") == 'all'){
                $('#make-car-m').hide();
                isshow = false;
            }
            else
                $("input[name=jpmall_year]").val($("#ul-car-year .selected3 a").attr("key"));
        }
...
//        $("#make-select-index").text(vechileName);
    ...
    $(function() {
        ...
        // 点击每一项 选中 makelist
        $("#ul-car-makes .li_list").click(function(e) {
            sericesarget = true;
            yeartarget = true;
            ...
            $.getJSON(Yii_jpdataUrl + '/vehicle/goodsSeries', {make: makeId}, function(result) {
                $('.car_list').show();
                $("#ul-car-series").append('<li class="li_all" id="series_first"><a  href="javascript:void(0)" key="all">所有车系</a></li>');
                if ($("#make-select").attr('key') == 'null_series' || $("#make-select-index").attr('key') == 'null_series') {
                    $("#series_first").hide();
                }
                $.each(result, function(index, value) {
                    $("#ul-car-series").append('<li class="li_list"><div  class="series_title"><a key=' + value.seriesId + ' href="javascript:void(0)" title='+value.name+' >' + value.name + '</a></div></li>');
                })
                if ($("#ul-car-series  li:first").length > 0)
                    sericesarget = true;
                $("#ul-car-series  li:eq(1)").click();
            })
        })
        // 点击每一项 选中 serieslist
        $("#ul-car-series .li_list").live('click', function(e) {
            yeartarget = true;
            var clas = $(this).attr('class');
            ...
            $.getJSON(Yii_jpdataUrl + '/vehicle/goodsYear', {seriesId: seriesId}, function(result) {
                //   console.log(result)
                $('.model_list').show();
                $("#ul-car-year").append('<li class="li_all"><a  href="javascript:void(0)" key="all">所有年款</a></li>');
                $.each(result, function(index, year) {
                    if (index != 0) {
                        ...
                        $("#ul-car-year").append(li);
                        yeartarget = true;
                        //                        vehiletarget = false;
                    }
                })
                $("#ul-car-year .li_list:eq(0)").click();
            })
        });
        // 点击每一项 选中 serieslist
        $("#ul-car-year .li_list").live('click', function(e) {
            var clas = $(this).attr('class');
            ...
            $.getJSON(Yii_jpdataUrl + '/vehicle/goodsYearModels', {seriesId: seriesId, year: year}, function(result) {
                ...
                $('.model_list').show();
                $("#ul-model").empty();
                $("#ul-model").append('<li class="li_all"><a  href="javascript:void(0)" key="all">所有车型</a></li>');
                $.each(result, function() {
                    $("#ul-model").append('<li class="li_list"><a  key=' + this.modelId + ' code=' + this.code + ' href="javascript:void(0)">' + this.name + '</a></li>');
                })
            })
        });
...
<?php endif; ?>
        ...
        //            $('#make-car-m').hide();
        //        });
        //        $("#make-car-m").live('mouseover',function(){
        //            $(this).show();
        //        });
        //        
        //        $("#make-car-m").live('mouseleave',function(){ // mouseout
        //            $(this).hide();
        //        });
        ...
        $(document).click(function(e) {
            ...
            if (!make_select) {
                ...
                if(isshow){
                    if ($("#make-select").val() ) {
                        $("#make-select").val('请选择汽车品牌');
                    } else {
                        $("#make-select-index").val('请选择汽车品牌');
                    }
                }
            }
            if(isshow){
        ...
        //            top = offset.top +26 + 'px';
        //          //  if($(this).val() != "请选择适用车系")
        //           //     $("#make-car-m").css({ 'left':left, 'top':top }).show();
        //            // $("#make-car-m").show();
        //        });
        //        $("#make-select").mouseout(function() {
        //            $("#make-car-m").hide();
        //            // $("#make-car-m").show();
        //        });
