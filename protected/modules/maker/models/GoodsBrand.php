<?php

/**
 * This is the model class for table "{{goods_brand}}".
 *
 * The followings are the available columns in table '{{goods_brand}}':
 * @property integer $id
 * @property string $name
 * @property string $car
 */
class GoodsBrand extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return GoodsBrand the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return '{{goods_brand}}';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('name', 'length', 'max'=>20),
			array('car', 'length', 'max'=>100),
			array('year', 'length', 'max'=>10),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, name, car', 'year', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'name' => 'Name',
			'car' => 'Car',
			'year' => 'Year',
		);
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria=new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('name',$this->name,true);
		$criteria->compare('car',$this->car,true);
		$criteria->compare('year',$this->car,true);

		return new CActiveDataProvider($this, array(
			'criteria'=>$criteria,
		));
	}
	public static function getBrand()
	{
		return	Yii::app()->db->createCommand()
			    ->select('*')
			    ->from(Goodsbrand::tableName())
			    ->group('name')
			    ->queryAll();
	} 
	public static function showName($id)
	{
		$model = self::model()->find("id=:id",array(":id"=>$id));
		echo $model->name;
	}
	public static function showCar($id)
	{
		$model = self::model()->find("id=:id",array(":id"=>$id));
		echo $model->car;
	}
	public static function getName($id)
	{
		$model = self::model()->find("id=:id",array(":id"=>$id));
		return $model->name;
	}
	public static function getCar($id)
	{
		$model = self::model()->find("id=:id",array(":id"=>$id));
		return $model->car;
	}
	public static function getYear($id)
	{
		$model = self::model()->find("id=:id",array(":id"=>$id));
		return $model->year;
	}
	public static function getId($name,$car,$year)
	{
//		$model = self::model()->find(array(
//			"condition"=>"name = $name and car = $car and year = $year"
//		)
//		
//		//"name=:name,car=:car,year=:year",array(":name"=>$name,":car"=>$car,":year"=>$year));
//		return $model->id;
	}
}