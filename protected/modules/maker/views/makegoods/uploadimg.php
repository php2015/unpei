...
//  }); 
  ...
  //保存
    function importgoods(){
        var urlimg=0;
        var url =" <?php echo Yii::app()->createUrl('maker/makegoods/Adding'); ?>";
    ...
    function xximages(goodsid,goodsno){
        ...
        var xxurl = Yii_baseUrl +"/maker/makegoods/Delimggoods";
            ...
            'onUploadSuccess':function(file, data, response){//每个文件上传完成时执行的函数 response是服务器返回的数据
                ...
                if(responeseDataObj && responeseDataObj.code == 200){
                            ...
                            if(data){
                                ...
                                $.each(data,function(index,val){
                                    var span = "<span class='showimages'><img  style='width:80px;height:80px;' src="+themurl+val.ImageUrl+">"+
                                     ...
                                     "<span name='urlimg' onclick='xxximage(this)' key="+val.ImageUrl+" imgname="+val.ImageName+" class='close icon-close-green xx'></span></span>";
                    ...
                    var span = "<span class='showimages'><img  style='width:80px;height:80px;' src="+src+">"+
                        ...
                        "<span name='urlimg' onclick='xxximage(this)'goodsid="+responeseDataObj.GoodsID+"  imgname="+responeseDataObj.ImgName+" key="+responeseDataObj.filename+" class='close icon-close-green xx'></span></span>";
                    $("#"+responeseDataObj.GoodsNO+" .showimglist-img").append(span);
