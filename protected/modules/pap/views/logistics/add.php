...
if ($logid) {
    $this->breadcrumbs = array(
        ...
        '物流管理' => Yii::app()->createUrl('pap/logistics/index'),
        '修改物流配送'
    );
} else {
    $this->breadcrumbs = array(
...
<div class="bor_back m-top" style="height:500px; position:relative">
    <p class="txxx">添加物流配送
        <span class="float_r" style="margin-right:20px ;*margin-top:-35px">
            <a href="<?php echo Yii::app()->createUrl('/pap/logistics/index'); ?>" class="color_blue" style="font-weight:400">返回列表</a>
        </span>
    </p>
...
<script>
    $(function(){
        ...
        $('#save').click(function(){
            ...
            var logid=$('#logid').val();
            if(logid)
                var url=Yii_baseUrl+'/pap/logistics/add/logid/'+logid;
            else
                var url=Yii_baseUrl+'/pap/logistics/add';
            $.post(url,data,function(res){
                if(res.res==1){
                    $("#reminddg").html('<span style="color:blue">物流配送添加成功!</span>');
                    $("#reminddg").dialog("open");
                    var url=Yii_baseUrl+'/pap/logistics/index'; 
