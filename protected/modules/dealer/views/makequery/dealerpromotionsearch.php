<div class='width998 content'>
	...
	<div>
		...
		<div id="tab1" class="form-list">
					<?php echo CHtml::beginForm('dealerpromotionsearch','get');?>
					<p class="form-row">
						<label class="label">关 键 词：</label>
						<?php if(empty($search['keywords'])) $search['keywords']="配件名称、配件品牌"?>
							<?php echo CHtml::textField('keywords',$search['keywords'],array('class'=>'width248 input','fuc'=>'s'));?>
						<label class="label">
							&nbsp;&nbsp;OE&nbsp;号：&nbsp;
						</label>
						<?php if(empty($search['OENO'])) $search['OENO']="输入OE号"?>
						<?php echo CHtml::textField('OENO',$search['OENO'],array('class'=>'width63 input','fuc'=>'s'))?>
						<label class="label">
							配件品类:</label>
				...
				?>		
						 <?php echo Chtml::dropDownList('cp_name',$search['cp_name'], $data_update,array(
						    		'class'=>'width118 select',
						    		'empty'=>'请选择品类',
						    ));?>
...
<!--						<label class="label">所在地区：	</label>-->
                ...
                echo CHtml::label('所在地区：','',array('class'=>'label'));
                echo CHtml::dropDownList('provice', $search['provice'], $state,
                            array(
                            'class'=>'width130 select',
                            'empty'=>'请选择省份',   
                            'ajax' => array(
                            'type'=>'GET', //request type
...
<?php 
              //  $city_update = $model->isNewRecord ? array() : $city;
                echo Chtml::dropDownList('city', $search['city'], $city_update,
                            array(
                            'class'=>'width130 select',
                            'empty'=>'请选择城市',
                            'ajax' => array(
			...
			<?php echo CHtml::dropDownList('make',$search['make'], $make ,array(
					'class'=>'width118 select',
					'empty'=>'请选择品牌',
					'ajax' => array(
                            'type'=>'GET', //request type
		    ...
		    <?php echo CHtml::dropDownList('car',$_GET['car'], $vehicleModel_update,array(
		    		'class'=>'width118 select',
		    		'empty'=>'请选择车系',
		    ));?>
		   ...
		   <?php // echo CHtml::link('取消查询',array('makequery/dealerpromotionsearch'),array('class'=>'btn-green btn-green-small active'))?>
		   <?php //endif;?>
					--></p>
					...
					<div id="ctable1">
						<table cellspacing=0 cellpadding=0 id="listSort">
							...
							<tbody>
								...
								<td><?php echo $i?></td>
									<td>
									<?php echo CHtml::link($model['organName'],array('/dealer/makequery/detail/dealer/'.$model['userID']),array('target'=>'_blank'))?>
									...
									<td><?php $add = Area::getCity($model['province']).Area::getCity($model['city']).Area::getCity($model['area']); echo F::msubstr($add)?></td>
									<td><?php // echo !empty($model['BusinessCate']) ? F::msubstr($model['BusinessCate'],0,0)  : "暂无"?>
									<?php $cpnames  = MakeOrganGroupRelation::model()->findAll("userID=".$model['userID']);
									//echo $model['userID'];
										$k=1;
									...
									<?php if(!empty($model['userID'])):?>
										<a target="_blank" href='<?php echo Yii::app()->createUrl('dealer/makequery/queryprogoods/userID')?><?php echo '/'.$model['userID']?>'>查看商品</a>
									<?php else:?>
									暂无
									<?php endif;?>
									</td>
...
//	});
	...
	$("#gopage").click(function(){
		var url = "<?php echo Yii::app()->createUrl('dealer/makequery/dealerpromotionsearch'); ?>";
