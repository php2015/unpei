...
class Ad extends CActiveRecord {
	...
	return F::baseUrl().'/js/holder.js/' . $width . 'x' . $height . '/text:' . $text;
    }
	...
	if (file_exists(dirname(F::basePath()).$img)) {
	    $img_thumb = F::baseUrl().ImageHelper::thumb(1000, 120, $img, array('method' => 'resize'));
	    $img_thumb_now = CHtml::image($img_thumb, $this->title);
	    return CHtml::link($img_thumb_now, $this->url, array('title' => $this->title));
	} else {
	    return CHtml::link(CHtml::image($this->getHolderJs('1000', '120')), $this->url, array('title' => $this->title));
	}
    }
