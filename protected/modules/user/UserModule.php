...
class UserModule extends CWebModule {
     ...
     * @var boolean
     */
    public $captcha = array('registration' => true, 'login' => true);
    ...
    public function init() {
        ...
        $this->setImport(array(
            'user.models.*',
            'member.models.OrganRmployees',
            'user.components.*',
            'maker.models.OrganPhoto',
...
//                    $mailer->Password=substr($val['value'], 5, -2);
        ...
        $mailer->IsHTML(true);
        if (!$mailer->Send()) {
            return "邮件发送失败:" . $mailer->ErrorInfo;
            exit;
        } else {
            return "ok";
        }
    ...
    public static function user($id = 0, $clearCache = false) {
        ...
        if ($id) {
            if (!isset(self::$_users[$id]) || $clearCache)
                self::$_users[$id] = User::model()->with('organ')->findbyPk($id);
