 ...
 */
class ReturnnotifyController extends Controller
{
    /**
     * 清空过滤器列表
    ...
    public function filters() {
     ...
     /**
     *    支付完成后返回的URL，在此只进行提示，不对订单进行任何修改操作,
      * 这里不严格验证，不改变订单状态
     *
     *    @author    Garbin
     *    @return    void
    ...
    function actionIndex() {
        ...
        //$order_info  =  JporderService::getOrder($order_id);
         $order_info = OrderreturnService::getOrder($order_id);
        if (empty($order_info)) {
            /* 没有该订单 */
            $this->show_warning('forbidden');
            Yii::app()->end();
...
//        $notify_result = $payment->verify_notify($order_info, true);
        $notify_result = $payment->return_notify($order_info, true);
        ...
        //echo '支付成功';
        $notify = $_GET;
        $redirectUrl = Yii::app()->createUrl('pap/dealerreturn/index');
