<div class='content-row-bnm mt1em'>
    <?php echo CHtml::beginForm(Yii::app()->createUrl('servicer/servicequery/empgoods', array('dealer' => $_GET['dealer'])), 'get'); ?>
    <p class='mt1em'>
        ...
        <?php
        echo CHtml::dropDownList('search[system_type]', $_GET['search']['system_type'], $system, array(
        ...
        <?php
        echo CHtml::dropDownList('search[cp_name]', $_GET['search']['cp_name'], $cpName_data, array(
            'class' => 'width110 select',
            'empty' => '请选择品类',
        ));
        ?>
        <label class='label label-inline-wa' >商品类别：</label>
<?php echo CHtml::dropDownlist('search[category]', $_GET['search']['category'], CHtml::listData($category, 'name', 'name'), array('class' => 'width118 select', 'empty' => '商品类别'))
?>
    <p class="mt1em">
