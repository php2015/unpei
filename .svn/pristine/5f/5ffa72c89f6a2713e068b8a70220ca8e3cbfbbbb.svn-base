...
//                $html = call_user_func(array($descendant, $name));
            ...
            foreach ($options as $op) {
                $id = isset($op['id']) ? $op['id'] : 'id';
                $op['url'] = isset($op['url']) && $op['url'] ? array($op['url'], $id => $descendant->id) : '';
                if (isset($op['htmlOptions']['submit'])) {
                    $op['htmlOptions']['submit'] = array($op['htmlOptions']['submit'], $id => $descendant->id);
                }
     ...
     * @return string
     */
    public function getSelectHtml($descendants, $select_name, $selected_id = 0, $name = 'name', $id = 'id', $separators = array('&nbsp;&nbsp;|--', '&nbsp;&nbsp;|--', '&nbsp;&nbsp;|--', '&nbsp;&nbsp;|--')) {
     ...
     * @return array
     */
    public function getSelectOptions($descendants, $name = 'name', $id = 'id', $separators = array('--|--', '--|--', '--|--', '--|--')) {
        $level = 1;
        $prefix = array('');
        $options = array();
        foreach ($descendants as $n => $descendant) {
            if (!$descendant->isRoot()) {
                ...
                $descendant_name = implode('', array_slice($prefix, 0, $descendant->level - $level)) . $descendant_name . ' ' . $descendant->{$name};
                $options[$descendant->{$id}] = substr($descendant_name, 2);
            } else {
                $options[$descendant->{$id}] = $descendant->{$name};
            }
        }
        return $options;
    }
