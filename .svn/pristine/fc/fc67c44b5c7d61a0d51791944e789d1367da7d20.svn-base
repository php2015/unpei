...
(function($) {
    ...
    var handlers = {
        ...
        onDialogOpen : function() {
            ...
            this.queueData.errorMsg       = '上传文件错误提示：';
        ...
        onSelect : function(file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    //					var replaceQueueItem = confirm('文件名为 "' + file.name + '" 正在上传.\n是否重新上传？');
                    //					if (!replaceQueueItem) {
                ...
                switch(errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            ...
                            this.queueData.errorMsg += '\n上传文件最大数量为 (' + settings.queueSizeLimit + ')。';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件名为 "' + file.name + '" ；超过最大容量(' + settings.fileSizeLimit + ')。';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件名为 "' + file.name + '" 是空的。';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件名为 "' + file.name + '" 是一个不被接受的文件类型(' + settings.fileTypeDesc + ').';
                        break;
                }
            }
