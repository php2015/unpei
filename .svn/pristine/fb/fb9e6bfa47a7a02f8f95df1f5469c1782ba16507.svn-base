<link rel="stylesheet" href="<?php echo F::themeUrl(); ?>/css/helpmaincategory.css">
<?php
...
<div id="help_category" class="easyui-dialog" style="width:770px;height:620px;" modal="true" closed="true">  
    <div class="con" >
        ...
        <div class="con1">
            ...
            <input type="text" id="cpname_value" value="标准名称/拼音首字母"  class="text">
...
<!--                    <p class="cxlb" key="前大照灯壳">车身及附件》外观件》前大照灯壳</p>
        ...
        <div class="con3">
            ...
            <p id="freused">
                <a href="javascript:;;" class="freused" cpnameid="1101001" cpname="空气滤清器" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">空气滤清器</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101024" cpname="机油滤清器" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">机油滤清器</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101011" cpname="空调滤清器" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">空调滤清器</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101017" cpname="燃油滤清器" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">燃油滤清器</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101033" cpname="正时皮带" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">正时皮带</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101004" cpname="蓄电池" subnameid="1100000" subname="常用件" bignameid="1100000" bigname="常用件">蓄电池</a> 丨 
<!--                <a href="javascript:;;" class="freused">制动片</a> 丨 -->
                <a href="javascript:;;" class="freused" cpnameid="1101035" cpname="（挡风玻璃）雨刮片" subnameid="1100000" subname="常用件" bignameid="1100000" bigname="常用件">雨刮片</a> 丨 
                <a href="javascript:;;" class="freused" cpnameid="1101031" cpname="火花塞" subnameid="1101000" subname="日常维护保养件" bignameid="1100000" bigname="常用件">火花塞</a></li></ul></p>

            <fieldset class="test"> 
                <legend>特殊子类说明</legend> 
                <ul> 
                    <li>(1) <span style="color:#40b240">车身及连接件：</span>车身及连接件：包含车身碰撞事故件、部分车身外观件及连接件。例如：保险杠、车灯总成等均在此子类中。
                    </li> 
...
<script>
    $(function(){
        $("#helpcate").click(function(){
            ...
            //            var left = 300+'px';
            //            var top = 100+'px';
            //            $("#wra").css({ 'left':left, 'top':top }).show();
            //          var w =  document.body.clientWidth;
            //          var h =  document.body.clientHeight;
            // $("#zzjs_net").css({ 'width':w, 'height':h }).show();
            $("#help_category").parent('div').addClass('help_category');
            $("#help_category").dialog('open').dialog('setTitle','标准名称选择指引');
        });
        
        $("#freused a").click(function(){
            // alert($(this).text());
            $("#cpname-select").val($(this).attr('bigname')+' '+$(this).attr('subname')+' '+$(this).attr('cpname'));
            $("input[name=jpmall_maincate]").val($(this).attr('bignameid'));
            $("input[name=jpmall_subcate]").val($(this).attr('subnameid'));
            $("input[name=jpmall_cpname]").val($(this).attr('cpnameid'));
            $("input[name=maincategory]").val($(this).attr('bigname'));
            $("input[name=subcategory]").val($(this).attr('subname'));
            $("input[name=cpnamecategory]").val($(this).attr('cpname'));
            // $("#wra").fadeOut(1000);
            $("#help_category").dialog('close')
        });
        $("#seach-cxjg p").live('click',function(){
            $("#cpname-select").val($(this).attr('bigname')+' '+$(this).attr('subname')+' '+$(this).attr('cpname'));
            $("input[name=jpmall_maincate]").val($(this).attr('bignameid'));
            $("input[name=jpmall_subcate]").val($(this).attr('subnameid'));
            $("input[name=jpmall_cpname]").val($(this).attr('cpnameid'));
        ...
        $("#cpname_search").live('click',function(){
            ...
            $.getJSON(url,{cpname:cpname_v},function(result){
                ...
                $.each(result,function(index,value){
                    if(value.subname){
                        if(value.bigname){
                            var  p = "<p class='cxlb' bigname="+value.bigname+" bignameid="+value.bignameid+" subname="+value.subname+" subnameid="+value.subnameid+" cpname="+value.cpname+" cpnameid="+value.cpnameid+" key="+value.cpname+">"+value.bigname+"》"+value.subname+"》"+value.cpname+"</p>";
                            $("#seach-cxjg").append(p);
                        }
                    }
                })
