...
<div class='width998 content_row'>
 ...
 <div id="show_organ">
    ...
    <div class='auto_height jgcx info content-rows15'>
        <div style=" width: 600px;height:375px; float:left; background: white">
            ...
            <div class='info-list' style=" overflow: hidden">
                ...
                <br>
                <span style="display:block;float:left;width:70px;">机构介绍：</span>
                <span style=" word-break: break-all;display: block;float: left;width: 470px;overflow:auto;"><?php echo $model['organIntroduction']; ?></span>
        ...
        <div class='right-side float-m   bg-white' style="height:375px;width:386px">
            ...
            <div class='info-list'>
                ...
                <p style=" clear: both"></p>
                <p><span style="float:left;display: block;width: 150px;">固定电话：<?php echo $model['ContactPhone'] ?></span></p>
                <p style=" clear: both"></p>
    ...
    <div class='jgcx content-rows15 bg-white' style="margin-bottom: 15px;line-height: 2em;padding: 10px">
            ...
            <div class='info-list'>
                <span style="font-weight: bold">主营品类</span>：
                     <?php if (!empty($cpnames)): ?>
                        <?php foreach ($cpnames as $k=>$showcpname): ?>
                            <?php echo $showcpname['BigName'].' '.$showcpname['SubName'].' '.$showcpname['CpName'] ?>
                            <?php if(isset($cpnames[$k+1]))echo '';?>
                        <?php endforeach; ?>
                    <?php endif; ?>
                <br>
                <span style="font-weight: bold">主营车系</span>：
                    ...
                    <?php if (!empty($dealerv)): ?>
                        <?php foreach ($dealerv as $k=>$showvehicle): ?>
                              <?php $makeName = D::queryGoodsMakeInfo($showvehicle['businessMake']); echo $makeName['makeName']; ?> <?php $carinfo = D::queryGoodsSeriesInfo($showvehicle['businessCar']); echo $carinfo['seriesName']; ?><?php echo $showvehicle['businessYear'] ? $showvehicle['businessYear'] :''; ?><?php $modelinfo = D::queryGoodsModelInfo($showvehicle['businessCarModel']); echo $modelinfo['modelName'] ?>
    ...
    <!--                <div class='width998 bg-white' style=" height: 250px;margin: 15px 0">
                            <div class="title title-dashed">
                                &nbsp;机构照片
                                    <i class='icon-arr2r-white display-ib'></i>
                            </div>
    ...
    <?php if (!empty($model['organPhoto'])): ?>
        <?php $src = F::uploadUrl() . $model['organPhoto'] ?>
                                        <img style='width:250px;height:180px;' alt="<?php echo $model['organName'] ?>" src="<?php echo $src ?>">
    <?php endif; ?>
                            </div>
                    </div>-->
