...
class Predis_Client {
    ...
    private function setupClient($options) {
        ...
        if ($this->_options->iterable_multibulk === true) {
            $this->_responseReader->setHandler(
                    Predis_Protocol::PREFIX_MULTI_BULK, new Predis_ResponseMultiBulkStreamHandler()
            );
        }
        if ($this->_options->throw_on_error === false) {
            $this->_responseReader->setHandler(
                    Predis_Protocol::PREFIX_ERROR, new Predis_ResponseErrorSilentHandler()
            );
        }
    }
    ...
    private function createConnection($parameters) {
        $params = $parameters instanceof Predis_ConnectionParameters ? $parameters : new Predis_ConnectionParameters($parameters);
    ...
    public function setProfile($serverProfile) {
        ...
        $this->_serverProfile = (is_string($serverProfile) ? Predis_RedisServerProfile::get($serverProfile) : $serverProfile
                );
    }
    ...
    public function getConnection($id = null) {
        if (!isset($id)) {
            ...
            return Predis_Shared_Utils::isCluster($this->_connection) ? $this->_connection->getConnectionById($id) : $this->_connection;
        }
    }
    ...
    private function initPipeline(Array $options = null, $pipelineBlock = null) {
        ...
        if (isset($options)) {
            if (isset($options['safe']) && $options['safe'] == true) {
                ...
                $pipeline = new Predis_CommandPipeline($this, $connection instanceof Predis_Connection ? new Predis_Pipeline_SafeExecutor($connection) : new Predis_Pipeline_SafeClusterExecutor($connection)
                );
            } else {
...
abstract class Predis_InlineCommand extends Predis_Command {
    ...
    public function serializeRequest($command, $arguments) {
        ...
        return $command . (count($arguments) > 0 ? ' ' . implode($arguments, ' ') . Predis_Protocol::NEWLINE : Predis_Protocol::NEWLINE
                );
    }
    ...
    public function flushPipeline() {
        if (count($this->_pipelineBuffer) > 0) {
            ...
            $this->_returnValues = array_merge(
                    $this->_returnValues, $this->_executor->execute($connection, $this->_pipelineBuffer)
            );
            $this->_pipelineBuffer = array();
        }
        return $this;
    ...
    public function execute($block = null) {
        ...
        try {
            ...
            for ($i = 0; $i < $sizeofReplies; $i++) {
                $returnValues[] = $commands[$i]->parseResponse($execReply[$i] instanceof Iterator ? iterator_to_array($execReply[$i]) : $execReply[$i]
                );
                unset($commands[$i]);
            }
        } catch (Exception $exception) {
...
class Predis_ConnectionParameters {
    ...
    public function __construct($parameters = null) {
        ...
        $this->_parameters = is_array($parameters) ? self::filterConnectionParams($parameters) : self::parseURI($parameters);
    }
