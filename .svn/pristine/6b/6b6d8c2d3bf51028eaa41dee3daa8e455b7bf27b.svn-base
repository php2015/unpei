...
<div id="inquiryorder_show" style="padding:30px; display:<?php echo $display ?>">
...
?></span>
    ...
    <div class="fitem">
        <?php if (!$model): ?>
            <a class='btn' href="<?php echo Yii::app()->createUrl('cim/discountset/inquiry/flag/set'); ?>" style="margin-left: 60px;">设置</a>
        <?php else: ?>
            <a class='btn' href="<?php echo Yii::app()->createUrl('cim/discountset/inquiry/flag/set'); ?>" style="margin-left: 60px;">修改</a>
        <?php endif; ?>
    </div>
</div>
<div id="inquiryorder_set" style="padding:30px; display:<?php echo $display == 'none' ? 'block' : 'none'; ?>">
    <form id="inquiry_fm" method="post">
        <div class="fitem" style="height:30px;">
            <span>支付宝担保折扣率：</span>
        ...
        <div class="fitem" style="margin-top: 15px;">
            ...
            <span><a class='btn' href="<?php echo Yii::app()->createUrl('cim/discountset/inquiry'); ?>">返回</a></span>
        </div>
    </form>
</div>
<script>
            ...
            if(window.confirm("您确定要保存吗?")){ 
                ...
                if(OrderAlipay && OrderLogis){
                    ...
                    if((isNaN(firstAlipay) || lastAlipay != '%') || (isNaN(firstLogis) || lastLogis != '%')){
                        ...
                        $.post(Yii_baseUrl + "/cim/discountset/inquiryorder",{OrderAlipay:OrderAlipay,OrderLogis:OrderLogis},function(result){
                            if(result.errorMsg){
                                $.messager.alert("提示",result.errorMsg,"warning");
                            }
                            else{
                                window.location.href = Yii_baseUrl + "/cim/discountset/inquiry";
                            }
                        });
                    }
                }
