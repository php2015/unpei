...
<?php $path = 'dealer/' . $OrganID . '/BrandPhoto/'; ?>
    $(function() {
        ...
        $('#one_<?php echo $val['ID'];?>').uploadify({
            ...
            'buttonImage': '<?php echo F::themeUrl(); ?>/images/sc.png', //设置按钮图片
            ...
            'onUploadSuccess': function(file, data, response) {//每个文件上传完成时执行的函数 response是服务器返回的数据
                ...
                if (res && res.code == 200) {
                    ...
                    var url = '<?php echo F::baseUrl(); ?>/member/company/savebrandphoto';
        ...
        ?>
            ...
            //删除图片
                    function deleteimg(obj) {
                        ...
                        var del_url = '<?php echo F::baseUrl(); ?>/upload/ftpdelfile';
                        $.post(del_url, {'path': path}, function(res) {
                            if (res.res == 1) {
                                var url = '<?php echo F::baseUrl(); ?>/member/company/delbrandshoto';
